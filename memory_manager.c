...
static int walkthrough(struct task_struct *paramTask, unsigned long address){
    pgd_t* pgd;
    p4d_t* p4d;
    pud_t* pud;
    pmd_t* pmd;
    ...
    pgd = pgd_offset(paramTask->mm, address); // get pgd from mm and the page address
...
void TraversePageTable(void) {
    ...
    while(vmas) { // traverse the virtual address spaces
       //PAGE_SIZE = 4096
        ...
        for(x = vmas->vm_start; x < (vmas->vm_end); x+= PAGE_SIZE) { // traverse the pages in task's virtual memory area
            walkthrough(task, x);
...
enum hrtimer_restart timer_restart_callback(struct hrtimer *timer) { 
    ...
    swap = 0;
	TraversePageTable();
	printk("PID [%d]: RSS = [%lu] KB, SWAP = [%lu] KB, WSS = [%lu] KB", pid, rss*4, swap*4, wss*4);
